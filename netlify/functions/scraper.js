...
exports.handler = async (event) => {
  try {
    ...
    if (response.status !== 200) {
      console.log(`Failed to fetch page: Status ${response.status}`); // Log the status
      return {
    ...
    console.log(`Found ${papers.length} papers for year ${year}`); // Log the number of papers found
    ...
    console.log(`Error in scraper: ${error.message}`); // Log the error
    return {
      statusCode: 500,
      body: JSON.stringify({
        creator: 'Chathura Hansaka',
